...
const FIXED_GIF_SIZE = "1920x917";
const FIXED_CENTER_TEXT_COLOR = "#FF2670";
const FIXED_GLITCH_COLOR1 = "#FF2670";
const FIXED_GLITCH_COLOR2 = "#FFFFFF";
...
function App() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      const parsedGifWidth = parseInt(FIXED_GIF_SIZE.split('x')[0], 10);
  ...
  }, [textColor, bgColor, speed]);

  const handleCaptureGIF = async () => {
    setIsDownloading(true);
    const canvas = canvasRef.current;
    const [gifWidth, gifHeight] = FIXED_GIF_SIZE.split('x').map(Number);
    const gif = new GIF({
      workers: 2,
      quality: 10,
